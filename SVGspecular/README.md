...
In $Fig. 3$ we apply the algorithm $(1)$ to generate the stop colors of the gradients. Each gradient corresponds to a different exponent $n$ applied in algorithm $(1)$ as an user input. The exponents are the same and the gradients are presented in the same order as the corresponding highlight curves of $cos^n(\alpha)$ in $Fig. 2.$ The gradients were automatically generated by our [SVGspecular.java](SVGspecularCode.md) program, the resulting paths were copied and pasted in [fig3.svg](fig3.svg),  edited on Illustrator, saved back to fig3.svg file, and used as is in $Fig. 3.$
